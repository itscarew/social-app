...
export default function FeedCard({ post, postId, authUserId }) {
    ...
    return (
        <div className="bg-white border rounded-sm mx-auto mb-8 ">
            <div className="flex items-center px-4 py-3">
                ...
                <div className="ml-3 ">
                    <Link href={`/${user?.username}`} className="text-sm font-semibold antialiased block leading-tight"> {user?.username} <span> &#x2022; {moment(user?.createdAt).startOf("hour").fromNow()} </span> </Link>
                    <p className="text-gray-600 text-xs block">{post.location} pp</p>
                </div>
            </div>
